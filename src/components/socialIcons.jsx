/* Filled social glyphs, keyed by network name. */
export const social = {
  facebook: (p) => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" {...p}>
      <path d="M22 12a10 10 0 1 0-11.56 9.88v-6.99H7.9V12h2.54V9.8c0-2.5 1.49-3.89 3.78-3.89 1.09 0 2.24.2 2.24.2v2.46h-1.26c-1.24 0-1.63.77-1.63 1.56V12h2.78l-.44 2.89h-2.34v6.99A10 10 0 0 0 22 12Z" />
    </svg>
  ),
  twitter: (p) => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" {...p}>
      <path d="M18.24 2H21l-6.55 7.48L22.5 22h-6.9l-4.6-6.02L5.7 22H3l7.06-8.06L2 2h7.08l4.16 5.5L18.24 2Zm-1.21 18h1.5L7.07 4h-1.6l11.56 16Z" />
    </svg>
  ),
  instagram: (p) => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" {...p}>
      <path d="M12 2.16c3.2 0 3.58.01 4.85.07 3.25.15 4.77 1.69 4.92 4.92.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.15 3.23-1.66 4.77-4.92 4.92-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-3.26-.15-4.77-1.7-4.92-4.92C2.17 15.58 2.16 15.2 2.16 12s.01-3.58.07-4.85C2.38 3.92 3.9 2.38 7.15 2.23 8.42 2.17 8.8 2.16 12 2.16Zm0 3.68A6.16 6.16 0 1 0 18.16 12 6.16 6.16 0 0 0 12 5.84Zm0 10.16A4 4 0 1 1 16 12a4 4 0 0 1-4 4Zm6.41-10.85a1.44 1.44 0 1 0 1.44 1.44 1.44 1.44 0 0 0-1.44-1.44Z" />
    </svg>
  ),
  youtube: (p) => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" {...p}>
      <path d="M23 12s0-3.27-.42-4.85a2.54 2.54 0 0 0-1.79-1.79C19.21 5 12 5 12 5s-7.21 0-8.79.36a2.54 2.54 0 0 0-1.79 1.79C1 8.73 1 12 1 12s0 3.27.42 4.85a2.54 2.54 0 0 0 1.79 1.79C4.79 19 12 19 12 19s7.21 0 8.79-.36a2.54 2.54 0 0 0 1.79-1.79C23 15.27 23 12 23 12Zm-13 3.5v-7l6 3.5-6 3.5Z" />
    </svg>
  ),
}
